...
import { Box, Flex, Heading, Group, Link, Image, Input, InputAddon, VStack, HStack, Text } from '@chakra-ui/react'
...
export default function RegisterForm() {
    ...
    return (
        <>
            <Flex minW={1080} width={'vw'} marginTop={'80px'} align={'center'} justify={'center'}>
                <Box bg={'white'} width={964} minW={964} borderRadius={8} paddingLeft={8} paddingRight={8} paddingBottom={3}
                    shadow={'0px 0px 8px -4px var(--shadow-color)'}
                    shadowColor={'black'}>
                    <VStack>
                        <HStack gap={8}>
                            <Image src={'../meowairline-logo.png'} height={32}/>
                            <Heading fontSize={'2xl'} color={'colorPalette.700'}>Register for Meow Airlines</Heading>
                        </HStack>
                        <HStack as={'form'} id={'register-form'} gap={6} width={'100%'} align='baseline'>
                            <VStack width={'33%'}>
                                <Text width={'100%'} borderBottomWidth={1} textAlign={'center'}>Account details</Text>
                                {/* 
                                    Change errorText based on error 
                                    invalid property makes the outline red    
                                */}
                                <Field label={'Username'} invalid={false} errorText={errorTexts.noError}>
                                ...
                                <Field label={'Password'} invalid={false} errorText={errorTexts.noError}>
                                    ...
                                    <InputGroup width={'inherit'} startElement={<LuLock />}>
                                        <PasswordInput placeholder={'Your password'} maxLength={32}></PasswordInput>
                                    </InputGroup>
                                </Field>
                                <Field label={'Confirm password'} invalid={false} errorText={errorTexts.noError}>
                                    {/* password length in range(8, 32) */}
                                    <InputGroup width={'inherit'} startElement={<LuLock />}>
                                        <PasswordInput placeholder={'Confirm password'} maxLength={32}></PasswordInput>
                                    </InputGroup>
                                </Field>
                            </VStack>

                            <VStack width={'69%'}>
                                <Text width={'100%'} borderBottomWidth={1} textAlign={'center'}>User Information</Text>
                                <HStack width={'100%'} align={'baseline'} gap={6}>
                                    <VStack width={'50%'}>
                                        ...
                                        */}
                                        <Field label={'Gender'}>
                                            <RadioGroup aria-required asChild>
                                                <HStack gap={6} position={'relative'} left={12} marginTop={2}>
                                        ...
                                        <Field label={'Date of birth'} marginTop={3}>
                                            <Input width={'inherit'} type={'date'}></Input>
                                        </Field>

                                        <SelectRoot collection={countries} onValueChange={(e) => setCountryCode(e.value)}>
                                        ...
                                        <Field label={'Phone'} invalid={false} errorText={errorTexts.noError}>
                                            <Group width={'inherit'} gap={0}>
                                                ...
                                                <Input placeholder={'Phone number'} type={'tel'} pattern={'[0-9]{12}'}></Input>
                                            </Group>
                                        </Field>

                                        <Button width={'100%'} marginTop={6} type={'submit'} formNoValidate
                                    ...
                                    <VStack width={'50%'}>
                                        <Field label={'Identification Method'}>
                                            <RadioGroup asChild>
                                                <HStack gap={6} position={'relative'} left={6} marginTop={2}>
                                                    <Radio value='nin'>National ID</Radio>
                                                    <Radio value='passport'>Passport ID</Radio>
                                                </HStack>
                                            </RadioGroup>
                                        </Field>
                                        <Field label={'Identification Number'} marginTop={3} invalid={false} errorText={errorTexts.noError}>
                                            <InputGroup width={'inherit'} startElement={<LiaIdCard />}>
